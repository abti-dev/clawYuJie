...
const SKILL_NAME = "clawYuJie-gen";
...
function printBanner() {
  console.log(`
${c("magenta", "┌─────────────────────────────────────────┐")}
${c("magenta", "│")}  ${c("bright", "ClawYuJie Gen")} - OpenClaw Skill Installer ${c("magenta", "│")}
${c("magenta", "└─────────────────────────────────────────┘")}

Add image generation superpowers to your OpenClaw agent!
Uses ${c("cyan", "Doubao Seedream")} via ${c("cyan", "Volcengine")} for image generation.
`);
}
...
async function getArkApiKey(rl) {
  logStep("2/7", "Setting up Volcengine API key...");

  const ARK_URL = "https://console.volcengine.com/ark/region:ark+cn-beijing/endpoint";

  log(`\nTo use Doubao Seedream, you need a Volcengine ARK API key.`);
  log(`${c("cyan", "→")} Get your key from: ${c("bright", ARK_URL)}\n`);

  const openIt = await ask(rl, "Open Volcengine in browser? (Y/n): ");

  if (openIt.toLowerCase() !== "n") {
    logInfo("Opening browser...");
    if (!openBrowser(ARK_URL)) {
      logWarn("Could not open browser automatically");
      logInfo(`Please visit: ${ARK_URL}`);
    }
  }

  log("");
  const arkKey = await ask(rl, "Enter your ARK_API_KEY: ");

  if (!arkKey) {
    logError("ARK_API_KEY is required!");
    return null;
  }
...
async function updateOpenClawConfig(arkKey) {
...
async function main() {
  ...
  try {
    ...
    // Step 2: Get ARK API key
    const arkKey = await getArkApiKey(rl);
    if (!arkKey) {
      rl.close();
      process.exit(1);
    }
    ...
    await updateOpenClawConfig(arkKey);
