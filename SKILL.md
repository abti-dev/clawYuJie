...
该脚本支持图生图 (Image-to-Image)，默认使用一个固定参考图（可用于保持人物一致性），也可以在第四个参数指定新的参考图 URL 或本地文件路径。

```bash
# 语法
./scripts/clawYuJie.sh "<PROMPT>" "<CHANNEL>" "<CAPTION>" ["<REFERENCE_IMAGE_URL_OR_PATH>"]
```
...
**示例 3 (指定参考图 - 本地文件):**
```bash
./scripts/clawYuJie.sh "变成动漫风格" "#general" "动漫版！" "./skill/assets/yujie.png"
```
